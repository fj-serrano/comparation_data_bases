...
def select_archives():
    ...
    for root, dirs, files_in_dir in os.walk(file_path_clean):
        ...
        if os.path.basename(root).startswith(study):
            ...
            if study_loop not in estudios:
                estudios.append(study_loop)
...
def main():
    files, output_path, estudios, counts_estudios = select_archives()
    df_total = files_to_df(files, counts_estudios)
